...
- `.claude/CLAUDE.md` should be a symlink to the matching `AGENTS.md` when the
  filesystem supports it
- `.github/copilot-instructions.md` should stay thin and derivative rather than
...
- deduplicate or relocate `AGENTS.md`, `.claude/CLAUDE.md`, or Copilot
  instructions
- make `AGENTS.md` the source of truth and project thinner instructions into
...
5. Rewrite or create `AGENTS.md` files so each node owns the intent for its
   semantic scope.
6. Replace Claude instruction files with symlinks to the matching `AGENTS.md`
   when appropriate, or with minimal pointer files when symlinks are not
   suitable.
7. Rewrite Copilot instructions as a thin projection back to the canonical
   `AGENTS.md`.
...
- if a symlink cannot be created because of platform or repo constraints, write a
  thin Claude-specific pointer file instead of failing the whole operation
...
- every Claude symlink resolves to the intended `AGENTS.md`
- Copilot instruction files still point to a valid canonical source
...
- create Claude links and thin Copilot projections with minimal ambiguity
